...
export default ExtendedVerticalLayout.extend({
    ...
    addInOut: function (input = true, txt) {
        ...
        if (txt) {
            ...
            port.setSemanticGroup("asympubkey");
            port.setColor("#47D7AC");
            port.setBackgroundColor("#47D7AC");
            port.setValue(txt);

            this.add(label);
        }
        else {
            // get all asymmetric public keys the actor knows
            let knowledge = JSON.parse(sessionStorage.knowledge);
            knowledge.asymmetricPrivateKey.forEach(element => {
                label = null;
                if (input && element.derivedPublicKey.knownBy.includes(sessionStorage.sender)) {
                    label = new LabelRight({
                        text: element.derivedPublicKey.value,
                        stroke: 0,
                        radius: 0,
                        bgColor: null,
                        padding: { left: 10, top: 3, right: 10, bottom: 5 },
                ...
                if (label) {
                    ...
                    port.setColor("#47D7AC");
                    port.setBackgroundColor("#47D7AC");
                    port.setValue(element.derivedPublicKey.value);
